...
function formatMigrationContent(guide: (typeof migrationGuides)[keyof typeof migrationGuides], section?: string): string {
    ...
    customTools: [
        // Angular-specific: get_migration_guide
        {
            ...
            parameters: {
                ...
                section: {
                    type: 'string',
                    description: "Optional section within the migration guide (e.g., 'breaking', 'deprecations', 'whatsnew')"
                }
            },
            handler: async (_data: ComponentsData, args: Record<string, unknown>) => {
                ...
                if (!version) {
                    ...
                    return {
                        content: [
                            {
                                type: 'text' as const,
                                text: JSON.stringify({ available_migrations: summary }, null, 2)
                            }
                        ]
                    };
                }
                ...
                const guide = migrationGuides[guideKey] || migrationGuides[`v${parseInt(version.replace('v', '')) - 1}_to_${version}` as keyof typeof migrationGuides];
