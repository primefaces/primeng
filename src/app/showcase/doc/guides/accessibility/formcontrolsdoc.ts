...
@Component({
    ...
    template: `
        <app-docsectiontext>
            <p>
                Native form elements should be preferred instead of elements that are meant for other purposes like presentation. As an
                example, button below is rendered as a form control by the browser, can receive focus via tabbing and can be used with space
                key as well to trigger.
            </p>
            ...
            <p class="doc-section-description mt-4">
                <i>tabindex</i> is required to make a div element accessible in addition to use a keydown to bring the keyboard support
                back. To avoid the overload and implementing functionality that is already provided by the browser, native form controls
                should be preferred.
            </p>
            <app-code [code]="code3" [hideToggleCode]="true"></app-code>
            <h3>Relations</h3>
            <p class="doc-section-description">
                Form components must be related to another element that describes what the form element is used for. This is usually
