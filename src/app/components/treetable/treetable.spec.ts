...
import { ContextMenuModule, ContextMenu } from '@alamote/primeng/contextmenu';
...
class TestTreeTableComponent {
    ...
    loadNodes(event) {
        setTimeout(() => {
            ...
            if (event.first === 249980) this.createLazyNodes(event.first, 20);
            else this.createLazyNodes(event.first, event.rows);
        }, 50);
    }
