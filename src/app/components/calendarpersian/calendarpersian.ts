...
export const CALENDAR_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => CalendarPersian),
    multi: true
};
...
@Component({
            ...
            >
                            ...
                            <div class="p-datepicker-header">
                                ...
                                <div class="p-datepicker-title">
                                    ...
                                    <span class="p-datepicker-decade" *ngIf="currentView === 'year'">
                                        <ng-container *ngIf="!decadeTemplate">{{ yearPickerValues()[0] | faNum }} - {{ yearPickerValues()[yearPickerValues().length - 1] | faNum }}</ng-container>
                                            ...
                                            >
                                                <ng-container *ngIf="date.otherMonth ? showOtherMonths : true">
                                                    <span
                                                        ...
                                                        pRipple
                                                    >
                                                        <ng-container *ngIf="!dateTemplate">{{ date.day | faNum }}</ng-container>
                ...
                <div class="p-timepicker" *ngIf="(showTime || timeOnly) && currentView === 'date'">
                    <div class="p-hour-picker">
                        ...
                        <span><ng-container *ngIf="currentHour < 10">0</ng-container>{{ currentHour | faNum }}</span>
                        <button
                            class="p-link"
                            type="button"
                            (keydown)="onContainerButtonKeydown($event)"
                    ...
                    <div class="p-minute-picker">
                        ...
                        <span><ng-container *ngIf="currentMinute < 10">0</ng-container>{{ currentMinute | faNum }}</span>
                        <button
                            class="p-link"
                            type="button"
                            (keydown)="onContainerButtonKeydown($event)"
                    ...
                    <div class="p-second-picker" *ngIf="showSeconds">
                        ...
                        <span><ng-container *ngIf="currentSecond < 10">0</ng-container>{{ currentSecond | faNum }}</span>
                        <button
                            class="p-link"
                            type="button"
                            (keydown)="onContainerButtonKeydown($event)"
    ...
    styleUrls: ['./calendarpersian.css']
})
export class CalendarPersian implements OnInit, OnDestroy, ControlValueAccessor {
    @Input() style!: any;
    ...
    @Input() get defaultDate(): JDate {
        return this._defaultDate;
    }

    set defaultDate(defaultDate: JDate) {
        this._defaultDate = defaultDate;

        if (this.initialized) {
            const date = defaultDate || new JDate();
    ...
    @Input() get minDate(): JDate {
        return this._minDate;
    }

    set minDate(date: JDate) {
        this._minDate = date;
    ...
    @Input() get maxDate(): JDate {
        return this._maxDate;
    }

    set maxDate(date: JDate) {
        this._maxDate = date;
    ...
    @Input() get disabledDates(): JDate[] {
        return this._disabledDates;
    }

    set disabledDates(disabledDates: JDate[]) {
    ...
    set showTime(showTime: boolean) {
        ...
        if (this.currentHour === undefined) {
            this.initTime(this.value || new JDate());
        }
    ...
    ngOnInit() {
        ...
        const date = this.defaultDate || new JDate();
        this.createResponsiveStyle();
        this.currentMonth = date.getMonth();
        this.currentYear = date.getFullYear();
    ...
    onMonthSelect(event: any, index: any) {
        if (this.view === 'month') {
    ...
    updateInputfield() {
        ...
        this.inputFieldValue = this.persianNumberService.toPersian(formattedValue);
    ...
    selectDate(dateMeta: any) {
        let date = new JDate(dateMeta.year, dateMeta.month, dateMeta.day);
    ...
    getDaysCountInMonth(month: number, year: number) {
        return this.jDateCalculatorService.jDaysInMonth(year, month);
    }
    ...
    isMonthSelected(month: any) {
        if (this.isComparable() && !this.isMultipleSelection()) {
            ...
            const selected = new JDate(this.currentYear, month, 1);
    ...
    isDateEquals(value: any, dateMeta: any) {
        if (value && value instanceof JDate) return value.getDate() === dateMeta.day && value.getMonth() === dateMeta.month && value.getFullYear() === dateMeta.year;
        else return false;
    }
    ...
    onMonthCellKeydown(event: any, index: any) {
    ...
    onYearCellKeydown(event: any, index: any) {
        const cell = event.currentTarget;
    ...
    navigateToMonth(prev: any, groupIndex: any) {
        if (prev) {
    ...
    updateTime() {
        ...
        value = value ? new JDate(value.getTime()) : new JDate();
    ...
    parseValueFromString(text: string): JDate | JDate[] | null {
    ...
    isValidDate(date: any) {
        return date instanceof JDate && ObjectUtils.isNotEmpty(date);
    }
    ...
    updateUI() {
        ...
        let val: JDate = this.defaultDate && this.isValidDate(this.defaultDate) && !this.value ? this.defaultDate : propValue && this.isValidDate(propValue) ? propValue : new JDate();
        ...
        switch (event.toState) {
            ...
            case 'visibleTouchUI':
                if (!this.inline) {
                    ...
                    (this.overlay as HTMLDivElement).setAttribute(this.attributeSelector, '');
                    this.appendOverlay();
                    this.updateFocus();
                    if (this.autoZIndex) {
    ...
    appendOverlay() {
        if (this.appendTo) {
            if (this.appendTo === 'body') document.body.appendChild(this.overlay as HTMLDivElement);
                ...
                if (literal) {
                    ...
                    switch (format.charAt(iFormat)) {
                        ...
                        case 'D':
                            ...
                            break;
                        case 'o':
                            output += formatNumber('o', Math.round((new JDate(date.getFullYear(), date.getMonth(), date.getDate()).getTime() - new JDate(date.getFullYear(), 0, 0).getTime()) / 86400000), 3);
                            break;
                        case 'm':
    ...
    parseDate(value: any, format: any) {
        ...
        let iFormat: any,
            ...
            extra,
            iValue = 0,
            shortYearCutoff = typeof this.shortYearCutoff !== 'string' ? this.shortYearCutoff : (new JDate().getFullYear() % 100) + parseInt(this.shortYearCutoff, 10),
            year = -1,
            month = -1,
            day = -1,
            doy = -1,
            ...
            if (literal) {
                ...
                switch (format.charAt(iFormat)) {
                    ...
                    case 'y':
                        ...
                        break;
                    case '@':
                        date = new JDate(getNumber('@'));
                        year = date.getFullYear();
                        month = date.getMonth() + 1;
                        day = date.getDate();
                        break;
                    case '!':
                        date = new JDate((getNumber('!') - this.ticksTo1970) / 10000);
                        year = date.getFullYear();
                        month = date.getMonth() + 1;
                        day = date.getDate();
                        break;
        ...
        if (year === -1) {
            year = new JDate().getFullYear();
        } else if (year < 100) {
            year += new JDate().getFullYear() - (new JDate().getFullYear() % 100) + (year <= shortYearCutoff ? 0 : -100);
        }
        ...
        date = this.daylightSavingAdjust(new JDate(year, month - 1, day));
