...
export class JDateCalculatorService {
    ...
    private breaks: number[] = [-61, 9, 38, 199, 426, 686, 756, 818, 1111, 1181, 1210, 1635, 2060, 2097, 2192, 2262, 2324, 2394, 2456, 3178];

    constructor(public validator: JDateValidatorService, private arithmetics: ArithmeticsService) {}
    ...
    public numberOfPassedGDays(gDate: Date): number {
        ...
        const gMonthDiv = this.arithmetics.div(gMonth, 6);
        const result = this.arithmetics.div((gYear + gMonthDiv) * 1461, 4) + this.arithmetics.div(153 * this.arithmetics.mod(gMonth + 17, 12) + 2, 5) + gDate.getDate() - 34840408;
        return result - this.arithmetics.div(this.arithmetics.div(gYear + gMonthDiv, 100) * 3, 4) + 752;
    }
     ...
     */
    public numberOfPassedJDays(jYear: number, jMonth: number, jDay: number): number {
        return this.numberOfPassedGDays(new Date(this.jalaliYearToGeorgianYear(jYear), 2, this.firstDayOfJYearInMarch(jYear))) + jMonth * 31 - this.arithmetics.div(jMonth + 1, 7) * (jMonth - 6) + jDay - 1;
    }
    ...
    public createGDateFromDays(numOfDays: number): Date {
        const j = 4 * numOfDays + 139361631 + this.arithmetics.div(this.arithmetics.div(4 * numOfDays + 183187720, 146097) * 3, 4) * 4 - 3908;
        const i = this.arithmetics.div(this.arithmetics.mod(j, 1461), 4) * 5 + 308;
        const gDay = this.arithmetics.div(this.arithmetics.mod(i, 153), 5) + 1;
        const gMonth = this.arithmetics.mod(this.arithmetics.div(i, 153), 12);
        const gYear = this.arithmetics.div(j, 1461) - 100100 + this.arithmetics.div(7 - gMonth, 6);
    ...
    public numOfGLeapYears(targetGYear: number): number {
        return this.arithmetics.div(targetGYear, 4) - this.arithmetics.div((this.arithmetics.div(targetGYear, 100) + 1) * 3, 4) - 150;
    }
    ...
    public convertToJalali(gDate: Date): ISimpleDate {
        ...
        if (numOfDayInJYear >= 0) {
            if (numOfDayInJYear <= 185) {
                // The first 6 months.
                jalaliMonth = this.arithmetics.div(numOfDayInJYear, 31);
                jalaliDay = this.arithmetics.mod(numOfDayInJYear, 31) + 1;
        ...
        jalaliMonth = 6 + this.arithmetics.div(numOfDayInJYear, 30);
        jalaliDay = this.arithmetics.mod(numOfDayInJYear, 30) + 1;
        return { year: jalaliYear, month: jalaliMonth, day: jalaliDay };
    }
  ...
  */
    private jalCalLeap(jYear: number) {
        ...
        if (jYear < jp || jYear >= this.breaks[bl - 1]) throw new Error('Invalid Jalaali year ' + jYear);
        ...
        if (jump && jump - n < 6) n = n - jump + this.arithmetics.div(jump + 4, 33) * 33;
        leap = this.arithmetics.mod(this.arithmetics.mod(n + 1, 33) - 1, 4);
        if (leap === -1) {
            leap = 4;
        }
  ...
  */
    public jDaysInMonth(year: number, month: number): number {
        year += this.arithmetics.div(month, 12);
        month = this.arithmetics.mod(month, 12);
        if (month < 0) {
            month += 12;
            year -= 1;
        }
