    ...
    template: `
        <ng-template ngFor let-col [ngForOf]="columns" let-lastCol="last">
            <th #headerCell [ngStyle]="col.style" [class]="col.styleClass" [style.display]="col.hidden ? 'none' : 'table-cell'" (click)="dt.sort($event,col)" [attr.colspan]="col.colspan" [attr.rowspan]="col.rowspan"
                ...
                <span class="ui-column-resizer" *ngIf="dt.resizableColumns && ((dt.columnResizeMode == 'fit' && !lastCol) || dt.columnResizeMode == 'expand')" (mousedown)="dt.initColumnResize($event, col.frozen)"></span>
...
export class DataTable implements AfterViewChecked,AfterViewInit,AfterContentInit,OnInit,OnDestroy,BlockableUI {
    ...
    public isFrozenColumnResizing: boolean;
        ...
        if(columnWidth + delta > parseInt(minWidth)) {
            if(this.columnResizeMode === 'fit') {
                ...
                if(newColumnWidth > 15 && nextColumnWidth > 15) {
                    ...
                    if(this.scrollable) {
                      let colGroups = this.domHandler.find(this.el.nativeElement, 'colgroup.ui-datatable-scrollable-colgroup');
                      let colGroup: any;
                        if(this.hasFrozenColumns()) {
                          if(this.isFrozenColumnResizing) {
                ...
                if(this.scrollable) {
                  let headerBoxes = this.domHandler.find(this.el.nativeElement, '.ui-datatable-scrollable-header-box');
                  const colGroups = this.domHandler.find(this.el.nativeElement, 'colgroup.ui-datatable-scrollable-colgroup');
                  let colGroup: any;
                  if(this.hasFrozenColumns()) {
                    if(this.isFrozenColumnResizing) {
                      colGroup = colGroups[0];
                    }else {
                      headerBoxes[1].children[0].style.width = containerWidth;
                      colGroup = colGroups[1];
                    }
                  }else {
                    headerBoxes[0].children[0].style.width = containerWidth;
                    colGroup = colGroups[0];
                  }
                  colGroup.children[resizeColumnIndex].style.width = newColumnWidth + 'px';
                }
