    ...
    template: `
        <ng-template [ngIf]="node">
            ...
            <li *ngIf="!tree.horizontal" [ngClass]="['p-treenode',node.styleClass||'', isLeaf() ? 'p-treenode-leaf': '']">
                <div class="p-treenode-content" [style.paddingLeft]="(level * indentation)  + 'rem'" (click)="toggle($event)" (click)="onNodeClick($event)" (contextmenu)="onNodeRightClick($event)" (touchend)="onNodeTouchEnd()"
                    ...
                    <button type="button" class="p-tree-toggler p-link"  pRipple tabindex="-1">
