...
export class JDate implements Date {
  ...
  constructor(
    ...
    if (!jYear) {
      ...
      this._createFromDate(
        JDate.changeTimeZone(new Date(JDate.parse(jYear)), timezone)
      );
      ...
      const gregorianDate = this._calculator.convertToGeorgian(
        jYear as number,
        jMonth as number,
        jDay as number
      );
  ...
  public static parse(dateString: string): number {
    ...
    const timeArray =
      dateArray.length > 3 ? dateArray[3].split(':') : ['0', '0', '0'];
  ...
  getUTCTime(): number {
    ...
    return (
      this._gregorianDate.getTime() +
      this._gregorianDate.getTimezoneOffset() * 60 * 1000
    );
  }
  ...
  private _renewGDate(): void {
    ...
    this._gregorianDate = this._calculator.convertToGeorgian(
      this._jalaliYear,
      this._jMonth,
      this._jDay
    );
  ...
  private _checkDateIsValid(): void {
    ...
    if (
      !this._calculator.validator.isValidJDate(
        this._jalaliYear,
        this._jMonth,
        this._jDay
      )
