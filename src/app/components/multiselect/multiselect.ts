import {
    ...
    forwardRef, ViewChild, ChangeDetectorRef, TemplateRef, ContentChildren, QueryList, ContentChild, ViewChildren
} from '@angular/core';
                            ...
                            <div class="ui-chkbox ui-widget">
                                <div class="ui-helper-hidden-accessible">
                                    <input #itemcb type="checkbox" readonly="readonly" [checked]="isSelected(option.value)" (focus)="focusedItemCheckbox=itemcb" (blur)="focusedItemCheckbox = null"
                                        [attr.aria-label]="option.label" [disabled]="option.disabled || (maxSelectionLimitReached && !isSelected(option.value))" (keydown)="onSelectBoxKeydown($event)">
                                </div>
...
export class MultiSelect implements OnInit, AfterViewInit, AfterContentInit, AfterViewChecked, OnDestroy, ControlValueAccessor {
    ...
    @ViewChildren('itemcb') itemcb: QueryList<any>;
    ...
    onSelectBoxKeydown(event) {
        const listItemsArray = this.itemcb.toArray();
        switch (event.which) {
            // escape
            case 27:
                this.hideAndFocus();
            ...
            // keyDown
            case 40:
                if (this.getIndexOfFocusedItem(listItemsArray) < listItemsArray.length - 2) {
                    listItemsArray[this.getIndexOfFocusedItem(listItemsArray) + 1].nativeElement.focus();
                    // align scroll onto focused item. otherwise sometimes its out of the window.
                    listItemsArray[this.getIndexOfFocusedItem(listItemsArray) + 1].
                        nativeElement.scrollIntoView(false);
                    event.preventDefault();
                } else {
                    if ( this.getIndexOfFocusedItem(listItemsArray) < listItemsArray.length - 1) {
                        listItemsArray[this.getIndexOfFocusedItem(listItemsArray) + 1].nativeElement.focus();
                    }
                }
                break;
            // keyUp
            case 38:
                if (this.getIndexOfFocusedItem(listItemsArray) > 0) {
                    listItemsArray[this.getIndexOfFocusedItem(listItemsArray) - 1].nativeElement.focus();
                }
                event.preventDefault();
                break;
    ...
    getIndexOfFocusedItem(listItems): number {
        let index = -1;
        listItems.forEach((element, i) => {
            if (element.nativeElement === this.focusedItemCheckbox) {
                index = i;
            }
        });
        return index;
