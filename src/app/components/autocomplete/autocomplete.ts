...
@Component({
    ...
    template: `
        <span [ngClass]="{'ui-autocomplete ui-widget': true, 'ui-autocomplete-dd': dropdown, 'ui-autocomplete-multiple': multiple}"
          ...
          <ul #multiContainer *ngIf="multiple"
          ...
          <i *ngIf="loading"
             class="ui-autocomplete-loader fa fa-circle-o-notch fa-spin fa-fw"></i>
          <button pButton *ngIf="dropdown" type="button"
                  icon="fa-fw fa-caret-down"
...
export class AutoComplete implements AfterViewInit, AfterViewChecked, DoCheck, ControlValueAccessor, OnDestroy, AfterContentInit {
    ...
    constructor(public el: ElementRef, public domHandler: DomHandler,
                public renderer: Renderer2, public objectUtils: ObjectUtils,
                public cd: ChangeDetectorRef, public differs: IterableDiffers) {
    ...
    ngAfterViewChecked() {
        ...
        if (this.highlightOptionChanged) {
            setTimeout(() => {
                const listItem = this.domHandler.findSingle(this.panelEL.nativeElement, 'li.ui-state-highlight');
                if (listItem) {
    ...
    align() {
        if (this.appendTo) {
          this.domHandler.absolutePosition(
            this.panelEL.nativeElement, (this.multiple ? this.multiContainerEL.nativeElement : this.inputEL.nativeElement));
        }else {
        ...
        if (this.forceSelection) {
            ...
            if (this.suggestions)  {
                for (const suggestion of this.suggestions) {
                    const itemValue = this.field ? this.objectUtils.resolveFieldData(suggestion, this.field) : suggestion;
    ...
    updateInputField() {
        const formattedValue = this.value ?
          (this.field ? this.objectUtils.resolveFieldData(this.value, this.field) || '' : this.value) : '';
